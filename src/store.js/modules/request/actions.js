export default{
  async addrequest(context,data){
      ...
      const response = await fetch(`https://vue-app-cdefa-default-rtdb.firebaseio.com/requests/${coachID}.json?auth=` +token,{
        method:'POST',
 ...
 async loadrequest(context){
  ...
  const userId = context.rootGetters.avtiveuser;
       const response =await fetch(`https://vue-app-cdefa-default-rtdb.firebaseio.com/requests/${userId}.json?auth=` + token)
       if(!response.ok){
        //spme logic
